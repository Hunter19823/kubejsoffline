        ...
        try {
            if (value instanceof EventGroupWrapper eventGroup) {
                ...
                for (EventHandler handler : eventHandlers) {
                    String uniqueName = handler.extra == null ? String.format("%s.%s(%s)", name, handler.name, handler.eventType.get().getSimpleName()) : String.format("%s.%s(%s, %s)", name, handler.name, handler.eventType.get().getSimpleName(), "???");

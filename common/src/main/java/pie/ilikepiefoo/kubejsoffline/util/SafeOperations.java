...
public class SafeOperations {
    ...
    public static String safeUniqueTypeName( Type type ) {
        ...
        if (type instanceof TypeVariable<?> variable) {
            ...
            final var componentType =
                tryGet(genericArrayType::getGenericComponentType).orElse(null);
    ...
    public static Type safeUnwrapReturnType( final Object obj ) {
        ...
        if (obj instanceof Method method) {
            return (Type) (
                tryGetFirst(method::getGenericReturnType, method::getReturnType).orElse(null)
            );
        }
        if (obj instanceof Parameter parameter) {
            return (Type) (
                tryGetFirst(parameter::getParameterizedType, parameter::getType).orElse(null)
            );
        }
     ...
     * if it is a field, it returns the name of the field, as well as the remapped name if it
     * exists,
     * if it is a method, it returns the name of the method, as well as the remapped name if it
     * exists,

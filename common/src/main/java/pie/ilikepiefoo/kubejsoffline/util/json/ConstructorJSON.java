...
public class ConstructorJSON {
    ...
    public static JsonArray of( @Nullable Constructor<?>[] constructors ) {
        if (constructors == null) return new JsonArray();
        JsonArray array = new JsonArray();
        for (var constructor : constructors) {
            var constructorJson = of(constructor);
            if (constructorJson != null) array.add(constructorJson);
        }
        return array;
    }
    ...
    public static JsonObject of( @Nullable Constructor<?> constructor ) {
        ...
        if (!object.has(JSONProperty.PARAMETERS.jsName)) // Throw out any constructors that have
        // broken parameters.
        {return null;}
        return object;
    }

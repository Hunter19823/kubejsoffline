...
public class TypeJSON {
    ...
    public static void attachGenericAndArrayData(
        @Nonnull final JsonObject object,
        @Nullable final Supplier<Type> typeSupplier
    ) {
        SafeOperations.tryGet(typeSupplier).ifPresent(( type ) -> attachGenericAndArrayData(
            object,
            type
        ));
    }
        ...
        if (type instanceof ParameterizedType parameterizedType) {
            SafeOperations.tryGet(parameterizedType::getRawType).ifPresent(( rawType ) -> {
                ...
                if (null != typeObject && 0 != typeObject.size()) {
                    object.addProperty(JSONProperty.RAW_PARAMETERIZED_TYPE.jsName,
    ...
    public static void addGenericData(
        ...
        @Nullable final Supplier<Type[]> typeArgs
    ) {
        ...
        if (0 < arguments.size()) {
            if (obj.has(JSONProperty.PARAMETERIZED_ARGUMENTS.jsName)) {
                ...
                final var existingArguments = obj.get(JSONProperty.PARAMETERIZED_ARGUMENTS.jsName)
                    .getAsJsonArray();
                if (existingArguments.size() != arguments.size()) {
                    throw new IllegalStateException("Parameterized arguments are being " +
                        "overwritten!");
                }
                for (int i = 0; i < existingArguments.size(); i++) {
                    if (existingArguments.get(i).getAsInt() != arguments.get(i).getAsInt()) {
                        LOG.info(
                            "Type '{}' has different parameterized arguments!",
                            obj.get(JSONProperty.TYPE_ID.jsName).getAsInt()
                        );
                        ...
                        for (int j = 0; j < existingArguments.size(); j++) {
                            LOG.info(
                                "Existing[{}]: {}",
                                j,
                                ClassJSONManager.getInstance()
                                    .getTypeData()
                                    .get(existingArguments.get(j).getAsInt())
                            );
                        }
                        for (int j = 0; j < arguments.size(); j++) {
                            LOG.info(
                                ...
                                ClassJSONManager.getInstance()
                                    .getTypeData()
                                    .get(arguments.get(j).getAsInt())
                            );
                        }

                        throw new IllegalStateException(
                            "Parameterized arguments are being overwritten!");
                    }
                }
            }

...
public class FieldJSON {
    ...
    public static JsonArray of( @Nullable Field[] fields ) {
        if (fields == null) return new JsonArray();
        JsonArray array = new JsonArray();
        for (var field : fields) {
            var fieldJson = of(field);
            if (fieldJson != null) array.add(fieldJson);
        }
        return array;
    }
    ...
    public static JsonObject of( @Nullable Field field ) {
        ...
        if (typeJson == null || !typeJson.has(JSONProperty.TYPE_ID.jsName)) return null;
        var object = new JsonObject();

        // Add the name of the field
        object.addProperty(JSONProperty.FIELD_NAME.jsName, SafeOperations.safeUnwrapName(field));
        object.addProperty(
            JSONProperty.FIELD_TYPE.jsName,
            typeJson.get(JSONProperty.TYPE_ID.jsName).getAsInt()
        );
        ...
        if (annotations.size() > 0) object.add(JSONProperty.FIELD_ANNOTATIONS.jsName, annotations);

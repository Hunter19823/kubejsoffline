...
public class MethodJSON {
    ...
    public static JsonArray of( @Nullable Method[] methods ) {
        if (methods == null) return new JsonArray();
        JsonArray object = new JsonArray();
        for (var method : methods) {
            var methodJson = of(method);
            if (methodJson != null) object.add(methodJson);
        }
        return object;
    }
    ...
    public static JsonObject of( @Nonnull Method method ) {
        ...
        if (temp != null) object.addProperty("name", temp);
        ...
        object.addProperty(
            JSONProperty.METHOD_RETURN_TYPE.jsName,
        ...
        if (!object.has(JSONProperty.PARAMETERS.jsName)) // Throw out any methods that have
        // brocken parameters.
        {return null;}
